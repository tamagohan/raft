defmodule RaftTest do
  ...
  test "should be follower as initial state and become candidate" do
    ...
    assert :sys.get_state(:peer1) == {:follower,  %Raft.State{peer: peer, term: 1, voted_for: nil, config: config}}

    :timer.sleep(1_000)
    assert :sys.get_state(:peer1) == {:candidate, %Raft.State{peer: peer, term: 2, voted_for: nil, config: config}}
